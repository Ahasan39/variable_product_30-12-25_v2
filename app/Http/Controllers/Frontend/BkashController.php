...
class BkashController extends Controller
{
    ...
    public function grant()
    {
        ...
        // Step 1: Always decode as associative array for safer access
        $responseData = json_decode($response, true);
    
        if (!is_array($responseData)) {
            Log::error('Bkash API response is not JSON: ' . $response);
            throw new \Exception('Invalid Bkash response format.');
        }
    
        if (!isset($responseData['statusCode']) || $responseData['statusCode'] !== "0000") {
            Log::error('Bkash token failed: ' . json_encode($responseData));
            throw new \Exception('Bkash token request failed: ' . ($responseData['statusMessage'] ?? 'Unknown error'));
        }
    
        if (!isset($responseData['id_token'])) {
            Log::error('Bkash token missing: ' . json_encode($responseData));
            throw new \Exception('Bkash token missing.');
        }
    ...
    {     
        ...
        $body_data = array(
            'mode' => '0011',
            'payerReference' => ' ',
            'callbackURL' => env('APP_URL').'/bkash/checkout-url/callback?orderId='.$orderId,
            'amount' => $amount,
            'currency' => 'BDT',
            'intent' => 'sale',
