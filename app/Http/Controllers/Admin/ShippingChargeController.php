...
{    
    ...
    {
        $data['shipping_charges'] = UpazilaShippingCharge::all();
        // dd($data['upazilas']);
        return view('backEnd.shippingcharge.upazila_wise', $data);
    }
    
    public function upazilaWiseUpdate(Request $request)
    {
        // dd($request);
        if(count($request->id) > 0){
            for($i=0; $i<count($request->id); $i++){
                $shipping_charge = UpazilaShippingCharge::find($request->id[$i]);
                if($shipping_charge){
