...
Route::get('/session/set/bn', function () {
    session(['bangla' => true]);
    Toastr::error('Shifted to Bengali', 'Success!');
    return redirect()->back();
})->name('bn.set');

Route::get('/session/unset/bn', function () {
    session(['bangla' => false]);
    Toastr::error('Shifted to English', 'Success!');
    return redirect()->back();
})->name('bn.unset');

Route::get('/chatbox', [MessageController::class, 'chatBox'])->name('chatbox.view');
Route::post('/send-guest-message', [MessageController::class, 'storeGuest'])->name('guest.message');
Route::get('/messages', [MessageController::class, 'fetchByGuestEmail']);
...
Route::group(['namespace'=>'Frontend', 'middleware' => ['ipcheck','check_refer']], function() {
    ...
    Route::post('direct/purchase', [FrontEndController::class, 'direct_purchase'])->name('direct.purchase');
...
Route::get('/get-cities/{divisionID}', [CustomerController::class, 'getCities']);
...
Route::group(['namespace'=>'Admin','middleware' => ['auth','lock','check_refer'],'prefix'=>'admin'], function() {
    ...
    Route::post('admin/bulk-courier/{slug}', [OrderController::class, 'bulk_courier'])->name('admin.bulk_courier');
    ...
    Route::get('shipping-charge-upazila-wise', [ShippingChargeController::class,'upazilaWise'])->name('shippingcharges.upazilaWise');
    Route::post('shipping-upazila-wise/update', [ShippingChargeController::class,'upazilaWiseUpdate'])->name('shippingcharges.upazilaWise.update');
...
Route::get('/admin/message', function () {
    ...
    return view('backEnd.message.index', compact('senders'));
})->name('admin.messages');


// Fetch all messages from a sender via email (AJAX)
Route::get('/admin/messages/{email}', function ($email) {
...
// Send admin reply
Route::post('/messages/reply', function (Request $request) {
    $request->validate([
...
Route::get('getDistrictsByDivision', [FrontendController::class,'getDistrictsByDivision'])->name('getDistrictsByDivision');
Route::get('getShippingChargesByDistrict', [FrontendController::class,'getShippingChargesByDistrict'])->name('getShippingChargesByDistrict');
